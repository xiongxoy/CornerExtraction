...
\begin{algorithm}[h]  % h是什么意思？以前查过，现在忘了
  ...
  \noindent{\bf{Task:}} Extract vertices of a polygon contour. \\
  \noindent{\bf{Input:}} The contour $\mathbf{C}$, number of vertices $n$. \\
  \noindent{\bf{Output:}} Coordinates of the $n$ vertices. \\
  \BlankLine
  \noindent{\bf{Main procedure:}} \\
  $hull$ = \FindConvexHull{$\mathbf{C}$} \;
  $l$ = number of points in $hull$ \;
  $vectors$ = empty list \;
  \For{$i\leftarrow 1$ \KwTo $l$}{
    \tcp*[h]{compute direction vector} \;
    $v = \mathbf{C}[(i+1)\mod l] - \mathbf{C}[i] $  \;
